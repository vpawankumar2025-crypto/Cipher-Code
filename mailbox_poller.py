...
def _run_retention_purge():
    deleted = db.purge_expired_cases()
    if deleted:
        print(f"[retention] purged {deleted} case(s) past the configured retention window")
...
def start_scheduler():
    ...
    # Privacy/compliance safeguard: auto-purge case data past RETENTION_DAYS
    # (see privacy_compliance.py). Runs once a day — retention is measured
    # in days, so there's no benefit to checking more often than that.
    scheduler.add_job(_run_retention_purge, "interval", hours=24, id="retention_purge")
    scheduler.start()
    return scheduler
