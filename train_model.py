...
def load_or_build_dataset() -> pd.DataFrame:
    if not os.path.exists(DATASET_PATH):
        print(f"{DATASET_PATH} not found — building the real+modern dataset first...")
        from build_real_dataset import build
        build(out_path=DATASET_PATH)
...
def train():
    ...
    print(
        "\nNOTE for judge Q&A: this dataset blends a real 34k-email academic corpus "
        "(Enron spam/ham, see build_real_dataset.py) with a disclosed synthetic "
        "supplement for modern OTP/KYC/BEC-style attacks the 2001 corpus predates. "
        "The hold-out accuracy above is a same-distribution split of THIS dataset, "
        "not an independent real-world benchmark -- say that plainly if asked, "
        "rather than quoting the number unqualified."
    )
