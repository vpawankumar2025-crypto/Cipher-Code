...
async def analyze_email(file: UploadFile = File(...), current_user=Depends(auth.get_current_user)):
    ...
    # Hash the exact bytes as received, before any parsing or normalization
    # touches them — this is the evidence-integrity anchor point. Everything
    # downstream (parsing, scoring, reports) traces back to this hash.
    evidence = evidence_integrity.hash_evidence(contents, filename=file.filename)
    chain_entry = evidence_integrity.append_chain(
        prev_chain_hash="", evidence_sha256=evidence["sha256"],
        event="ingested", detail=f"uploaded by user {current_user['sub']}",
    )
    ...
    try:
        ...
        case_id = db.save_case(parsed, geo, verdict_data, threat_intel_result,
                                ...
                                evidence=evidence, chain_entry=chain_entry)
        # Real-time push alert for Malicious/high-score verdicts. Fires on a
        # background thread, so a slow or failing Twilio/SMTP call can never
        # delay or break this request — the case is already saved either way.
        alert_result = alerts.maybe_alert(
            case_id, verdict_data, parsed, geo, user_id=int(current_user["sub"])
        )
        scored_entry = evidence_integrity.append_chain(
            prev_chain_hash=chain_entry["chain_hash"], evidence_sha256=evidence["sha256"],
            event="scored", detail=f"verdict={verdict_data['verdict']} score={verdict_data['score']}",
        )
        db.append_evidence_chain(case_id, scored_entry)
    finally:
        os.unlink(tmp_path)
    ...
    return {
        ...
        "explanation": verdict_data.get("ml_explanation", {}),
        "alert": alert_result,
        "typosquat": verdict_data.get("typosquat", {}),
        "evidence": {
            "sha256": evidence["sha256"],
            "size_bytes": evidence["size_bytes"],
            "hashed_at": evidence["hashed_at"],
        },
    }
...
def get_case_detail(case_id: int, current_user=Depends(auth.get_current_user)):
    ...
    case = db.get_case(case_id, user_id=user_id, is_admin=is_admin)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    db.record_audit(user_id, "view_case", case_id=case_id)
    is_owner = case.get("user_id") == user_id
    return privacy_compliance.mask_case_for_view(case, viewer_is_admin=is_admin, viewer_is_owner=is_owner)
...
def override_case_verdict(case_id: int, body: OverrideRequest, current_user=Depends(auth.get_current_user)):
    is_admin = current_user.get("role") == "admin"
    user_id = int(current_user["sub"])
    success = db.override_verdict(case_id, body.verdict, user_id=user_id,
                                   is_admin=is_admin, notes=body.notes)
    if not success:
        raise HTTPException(status_code=404, detail="Case not found")
    db.record_audit(user_id, "override_verdict", case_id=case_id, detail=f"-> {body.verdict}: {body.notes}")
...
def download_case_report(case_id: int, current_user=Depends(auth.get_current_user)):
    ...
    db.record_audit(int(current_user["sub"]), "download_report", case_id=case_id)
    return FileResponse(out_path, media_type="application/pdf",
                         filename=f"SentinelMail_Case_{case_id}_Report.pdf")


@app.get("/api/cases/{case_id}/incident-report")
def download_incident_report(case_id: int, current_user=Depends(auth.get_current_user)):
    """Pre-filled CERT-In / cybercrime.gov.in incident report as a PDF.
    A DRAFT for a human to review and sign — this files nothing anywhere."""
    is_admin = current_user.get("role") == "admin"
    case = db.get_case(case_id, user_id=int(current_user["sub"]), is_admin=is_admin)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")

    out_path = os.path.join(tempfile.gettempdir(), f"case_{case_id}_incident_report.pdf")
    cert_in_report.generate_cert_in_pdf(case, out_path)
    db.record_audit(int(current_user["sub"]), "download_incident_report", case_id=case_id)
    return FileResponse(out_path, media_type="application/pdf",
                         filename=f"IncidentReport_Case_{case_id}.pdf")


@app.get("/api/cases/{case_id}/incident-report.json")
def incident_report_json(case_id: int, current_user=Depends(auth.get_current_user)):
    """Same report as structured JSON — for machine-to-machine sharing with a
    CSIRT/ISAC, or to pre-fill a web form."""
    is_admin = current_user.get("role") == "admin"
    case = db.get_case(case_id, user_id=int(current_user["sub"]), is_admin=is_admin)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    return cert_in_report.build_incident_report(case)
...
def get_dashboard_analytics(days: int = 14, current_user=Depends(auth.get_current_user)):
    """V2 threat analytics: trends, top domains/countries and high-risk alerts."""
    is_admin = current_user.get("role") == "admin"
    return db.dashboard_analytics(user_id=int(current_user["sub"]), is_admin=is_admin, days=days)


@app.get("/api/cases/{case_id}/evidence-verify")
def verify_case_evidence(case_id: int, current_user=Depends(auth.get_current_user)):
    """Recomputes the case's hash chain and reports whether it's intact.
    A broken chain means a record was altered or deleted after the fact —
    this is the concrete answer to 'how do you know it wasn't tampered with'."""
    is_admin = current_user.get("role") == "admin"
    case = db.get_case(case_id, user_id=int(current_user["sub"]), is_admin=is_admin)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    chain = case.get("evidence_chain") or []
    result = evidence_integrity.verify_chain(chain)
    result["evidence_sha256"] = case.get("evidence_sha256")
    result["chain_length"] = len(chain)
    return result


@app.get("/api/dashboard/attack-map")
def get_attack_map(days: int = 30, malicious_only: bool = False,
                   current_user=Depends(auth.get_current_user)):
    """Geolocated attack origins for the world map."""
    is_admin = current_user.get("role") == "admin"
    return db.attack_map(user_id=int(current_user["sub"]), is_admin=is_admin,
                         days=days, malicious_only=malicious_only)


@app.get("/api/campaigns")
def get_campaigns(days: int = 30, current_user=Depends(auth.get_current_user)):
    """Groups related Malicious/Suspicious cases sharing sender domain +
    originating IP into campaigns — the forensic case-management view
    named explicitly in the SIH26106 brief."""
    is_admin = current_user.get("role") == "admin"
    return db.list_campaigns(user_id=int(current_user["sub"]), is_admin=is_admin, days=days)


@app.get("/api/cases/{case_id}/advanced")
def get_advanced_case_analysis(case_id: int, current_user=Depends(auth.get_current_user)):
    """Explainable AI, IOC extraction, header forensics and relationship graph."""
    is_admin = current_user.get("role") == "admin"
    advanced = db.advanced_case(case_id, user_id=int(current_user["sub"]), is_admin=is_admin)
    if advanced is None:
        raise HTTPException(status_code=404, detail="Case not found")
    return advanced
...
def signup(body: SignupRequest):
    ...
    # Email-only verification now (no mobile collected at signup). mark_channel_verified
    # already treats mobile as "not required" whenever the account has none on file.
    auth.issue_otp(user.email, channel="email", purpose="signup")
    return {
        "message": "Signup successful. Enter the OTP code sent to your email to verify your account.",
        "user_id": user.id,
...
def login(body: LoginRequest):
    ...
    # Password verified successfully — now issue OTP for 2FA
    auth.issue_otp(user.email, channel="email", purpose="login")
    return {
        "message": "Password verified. Enter the OTP code sent to your email to complete login.",
        "requires_otp": True,
...
# ---------------------- Static dashboard ----------------------
# Mounted LAST, after every route above is registered — a mount declared
# earlier would shadow routes defined after it.
#
# Serves ONLY the static/ directory, never the project root: mounting the
# root would publish .env, the SQLite database and the model pickles over
# HTTP to anyone who guesses the filename.
#
