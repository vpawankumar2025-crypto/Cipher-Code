...
def _node_risk_color(score):
    """Mirrors the dashboard's riskColor() thresholds."""
    score = score or 0
    if score >= 60:
...
def _build_relationship_graph_drawing(graph, max_width=470):
    """Renders the same node/edge attack-relationship graph shown on the
    dashboard's 'Attack relationship graph' card, as a vector Drawing that
    can be dropped straight into the PDF story."""
    ...
    for i, n in enumerate(nodes):
        col = i % cols
        row = i // cols
        cx = col_w / 2 + col * col_w
        cy = height - (row_h / 2 + row * row_h)  # PDF y-axis grows upward
        pos[n.get("id")] = (cx, cy)

    # Edges (and their labels) drawn first so nodes sit on top.
    for e in edges:
        a = pos.get(e.get("source"))
        b = pos.get(e.get("target"))
        if not a or not b:
            continue
        d.add(Line(a[0], a[1], b[0], b[1], strokeColor=colors.HexColor("#9aa4b2"), strokeWidth=1))
        label = e.get("label")
        if label:
            mx, my = (a[0] + b[0]) / 2, (a[1] + b[1]) / 2
            d.add(String(mx, my + 3, str(label), fontSize=6.5,
                         fillColor=colors.HexColor("#697583"), textAnchor="middle"))

    # Nodes.
    for n in nodes:
        cx, cy = pos[n.get("id")]
        color = _node_risk_color(n.get("risk"))
        circle = Circle(cx, cy, radius, strokeColor=color, fillColor=color, strokeWidth=1.3)
        circle.fillOpacity = 0.16
        d.add(circle)
        label = str(n.get("label", ""))[:15]
        label_font_size = 7.5 if len(label) <= 10 else 5.8
        d.add(String(cx, cy + 2, label, fontSize=label_font_size,
                     fillColor=colors.HexColor("#1a1f26"), textAnchor="middle"))
        d.add(String(cx, cy - 10, str(n.get("type", "")), fontSize=6.5,
                     fillColor=colors.HexColor("#5c6472"), textAnchor="middle"))
...
def generate_report(parsed: dict, geo: dict, verdict_data: dict, output_path: str, case_meta: dict = None):
    ...
    graph = (case_meta.get("advanced") or {}).get("graph") or {}
    graph_drawing = _build_relationship_graph_drawing(graph)
    if graph_drawing is not None:
        elements.append(Paragraph("Attack Relationship Graph", heading_style))
        elements.append(Paragraph(
            "Node color reflects that entity's individual risk score (green = low, amber = "
            "medium, red = high); edge labels describe the relationship between entities.",
            small_grey))
        elements.append(Spacer(1, 6))
        elements.append(graph_drawing)
        elements.append(Spacer(1, 12))
